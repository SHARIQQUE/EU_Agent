...
import adapter.ReportsCommissionAdapter;
...
public class CommissionPeriod extends AppCompatActivity implements View.OnClickListener, TextView.OnEditorActionListener, View.OnTouchListener, ServerResponseParseCompletedNotifier, DateSetNotifier, TaskCompleteToPrint {
    ...
    String readerAddress, printerAddress,totalFees;
    private Printer mPrinter;
    Double totalFeesAdd;

    int hourSet1, minuteSet1,hourSet2,minuteSet2;
    String totalFessAddString,fromTimeSetString, toTimeSetString, amPmSelect1,amPmSelect2;
    ...
    CommissionPeriod.ConnectToPrinter connect_to_Printer;
    String printdata;
    ArrayList<String> responseListForPrinter_Header, responseListForPrinter_Body,feestotal_list,responseListForPrinter_TotalFees;
    ...
    LinearLayout linearlayout_printButton,linearlayout_totalamountRecepit;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.commission_period);
        mComponentInfo = (ComponentMd5SharedPre) getApplicationContext();
        ...
        linearlayout_totalamountRecepit = (LinearLayout) findViewById(R.id.linearlayout_totalamountRecepit);
                    ...
                    if (hourSet2>0 || minuteSet2>0) {
                        ...
                        if (isDateInOrder(fromDateString, toDateString)) {
                            ...
                            */
                                ...
                                Toast.makeText(CommissionPeriod.this, getString(R.string.prompt_mPin), Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseentercorrectdates), Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(CommissionPeriod.this, getString(R.string.selectToTime), Toast.LENGTH_SHORT).show();
                    }

                } else {
                    Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseentertodate), Toast.LENGTH_SHORT).show();
                }
            } else {
                Toast.makeText(CommissionPeriod.this, getString(R.string.selectFromTime), Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseenterfromdate), Toast.LENGTH_SHORT).show();
        }
    ...
    public void validateDetails_NextButton() {
        if (isMiniStmtMode) {
            ...
            linearlayout_totalamountRecepit.setVisibility(View.GONE);
    ...
    void printRequestProcessing() {
        if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
            if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                connect_to_Printer = new CommissionPeriod.ConnectToPrinter();
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(CommissionPeriod.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, CommissionPeriod.this);
                        ...
                        a++;
                        status = 3;
                        CommissionPeriod.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(CommissionPeriod.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    CommissionPeriod.PrintToPrinter printToPrinter = new CommissionPeriod.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(CommissionPeriod.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = CommissionPeriod.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            PrintUtils mPrintUtils = new PrintUtils(CommissionPeriod.this, mPrinter, bitMap, CommissionPeriod.this);
            mPrintUtils.print_Header_Reports_commisionReport(responseListForPrinter_Header);
            mPrintUtils.print_reportHistory_commisionReport(responseListForPrinter_Body);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(CommissionPeriod.this);
        try {
    ...
    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(CommissionPeriod.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(CommissionPeriod.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(CommissionPeriod.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
    ...
    void proceedMiniStmt() {
        if (new InternetCheck().isConnected(CommissionPeriod.this)) {
            showProgressDialog(getString(R.string.pleasewait));
         ...
         //   callApi("getAgentReportComm",requestData,162);
            new ServerTask(mComponentInfo, CommissionPeriod.this, mHandler, requestData, "getAgentReportComm", 162).start();
        } else {
            Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseCheckInternet), Toast.LENGTH_LONG).show();
        }
    }
            ...
            JsonObjectRequest jsObjRequest = new JsonObjectRequest(Request.Method.POST,getString(R.string.url)+apiName,new JSONObject(body),
                    ...
                    new Response.Listener<JSONObject>() {
                        ...
                        public void onResponse(JSONObject response) {
                            ...
                            DataParserThread thread = new DataParserThread(CommissionPeriod.this,mComponentInfo,CommissionPeriod.this,requestCode,response.toString());
                    ...
                    new Response.ErrorListener() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            hideProgressDialog();
                            Toast.makeText(CommissionPeriod.this,getString(R.string.pleaseTryAgainLater),Toast.LENGTH_LONG).show();
    ...
    private String generateMiniSttData() {
        ...
        try {
            ...
            /*
             ...
             {
               "agentcode":"237679525866",
               "pin":"007F4B75FA679BB8A8AAF0F9914D01E7",
               "clienttype":"GPRS",
               "vendorcode":"MICR",
               "accounttype":"MA",
               "pintype":"IPIN",
               "criteria":[
                  {
                     "transtype":"REMTSEND",
                     "datefrom":"11/11/2010 2:45:37 PM",
                     "dateto":"11/12/2010 2:10:13 PM",
                     "count":"5"
                  }
               ]
          }
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message message) {
            ...
            if (message.obj.toString().equals("Internet")) {
                hideProgressDialog();
                Toast.makeText(CommissionPeriod.this, getString(R.string.pleaseCheckInternet), Toast.LENGTH_SHORT).show();
                ...
                DataParserThread thread = new DataParserThread(CommissionPeriod.this, mComponentInfo, CommissionPeriod.this, message.arg1, message.obj.toString());
            ...
            if (generalResponseModel.getResponseCode() == 0) {
                ...
                if (requestNo == 162) {
                    ...
                    if (generalResponseModel.isOTP()) {
                        ...
                        Intent i = new Intent(CommissionPeriod.this, OTPVerificationActivity.class);
                        ...
                        CommissionPeriod.this.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (generalResponseModel.getCustomResponseList().size() > 0) {
                                    ...
                                    feestotal_list = new ArrayList<String>();
                                    feestotal_list=generalResponseModel.getCustomResponseList();
                                    feestotal_list = new ArrayList<String>();


                                    for (int k = 0; k < generalResponseModel.getCustomResponseList().size(); k++) {

                                        feestotal_list.add(k, generalResponseModel.getCustomResponseList().get(k).toString());

                                        String[] tempData = feestotal_list.get(k).split("\\|");


                                        tempDataFees= Double.parseDouble(tempData[1]);
                                         feesAddTotal=feesAddTotal+tempDataFees;

                                       // Double d=Double.parseDouble(feesAddTotal);
                                       // totalFeesAdd = Double.parseDouble(d);
                                        System.out.println(feesAddTotal);
                                        System.out.println(feesAddTotal);
                                    }
                                    System.out.println(feesAddTotal);

                                    ReportsCommissionAdapter adapter = new ReportsCommissionAdapter(CommissionPeriod.this, generalResponseModel.getCustomResponseList(), mComponentInfo.getmSharedPreferences().getBoolean("isOtherAccount", false));
                                    ...
                                    totalFessAddString = new Double(feesAddTotal).toString();
                                    textView_totalamountRecepit.setText(totalFessAddString);

                                    responseListForPrinter_TotalFees = new ArrayList<String>();
                                    responseListForPrinter_TotalFees.add(0, totalFessAddString);


                                    linearlayout_totalamountRecepit.setVisibility(View.VISIBLE);
                                    nextButton_MiniStatement.setText(getString(R.string.searchanother));

                                    isMiniStmtMode = true;
                                } else {

                                    Toast.makeText(CommissionPeriod.this, getString(R.string.norecordfound), Toast.LENGTH_SHORT).show();
                ...
                hideProgressDialog();
                Toast.makeText(CommissionPeriod.this, "" + generalResponseModel.getUserDefinedString(), Toast.LENGTH_SHORT).show();
