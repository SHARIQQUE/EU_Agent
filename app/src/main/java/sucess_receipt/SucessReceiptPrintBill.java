...
public class SucessReceiptPrintBill extends AppCompatActivity implements TaskCompleteToPrint {
    TextView dateRecepit, timeReceipt, totalamountRecepit, senderNameReceipt, CustomerIdReceipt,customerNumbrTextView, commentsTitleReview, customerNameReceipt, destinationNumberReceipt, transactionCodeRecepit, transactionIdReceipt, marchantNameReceipt,customerNumbReceipt, billNumberReceipt, agentBranceNameReceipt, cityReceipt, labelNameReceipt, amountRecepit, feesRecepit, agentNameRecepit, agentCountryRecepit;
    Button homeButton;
    Double feesNull,feesAdd,totalAmount,amountAdd;
    String totalAmountString;
    String spinnerCountryString;
    String[] recieptData;
    ComponentMd5SharedPre mComponentInfo;
    String agentCode, agentName;
     LinearLayout linearlayout_customerNumber;
    String[] recieptDataDoller,customerNumberArray;
    ...
    String[] billNumber,arrayData;
    SucessReceiptPrintBill.ConnectToPrinter connect_to_Printer;
    String printdata;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sucess_receipt_billpay);
        Toolbar mToolbar;
        ...
        customerNumbrTextView = (TextView) findViewById(R.id.customerNumbrTextView);

        linearlayout_customerNumber=(LinearLayout)findViewById(R.id.linearlayout_customerNumber);
        ...
        arrayData = recieptData[9].split("\\$");
        CustomerIdReceipt.setText(recieptData[8]);

        transactionIdReceipt.setText(recieptData[15]);
        String dateTimeSaperate = recieptData[12];
        ...
        agentNameRecepit.setText(recieptData[0]);
        agentBranceNameReceipt.setText(recieptData[3]);
        marchantNameReceipt.setText(recieptData[6]);

        labelNameReceipt.setText(recieptData[7]);
        customerNameReceipt.setText(recieptData[6]);


        amountRecepit.setText(arrayData[1]);  // amount

         amountAdd = Double.parseDouble(arrayData[1]);

        if(recieptData[10].equalsIgnoreCase("null"))
        {
            feesNull=0.0;
            feesRecepit.setText("0.0");   // from server if fees null then display 0.0
            totalAmount = (amountAdd - feesNull);
            totalAmountString = Double.toString(totalAmount);
            totalamountRecepit.setText(totalAmountString);
        }
        else
        {
            feesRecepit.setText(recieptData[10]);
            feesAdd = Double.parseDouble(recieptData[10]);
        ...
        billNumber = recieptData[9].split("\\$");
        billNumberReceipt.setText(billNumber[0]);

        customerNumberArray = recieptData[5].split("\\$");
        linearlayout_customerNumber.setVisibility(View.VISIBLE);
        customerNumbrTextView.setText(customerNumberArray[1]);
        ...
        homeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SucessReceiptPrintBill.this.finish();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
                    if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                        connect_to_Printer = new SucessReceiptPrintBill.ConnectToPrinter();
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(SucessReceiptPrintBill.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, SucessReceiptPrintBill.this);
                        ...
                        a++;
                        status = 3;
                        SucessReceiptPrintBill.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(SucessReceiptPrintBill.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    SucessReceiptPrintBill.PrintToPrinter printToPrinter = new SucessReceiptPrintBill.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(SucessReceiptPrintBill.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = SucessReceiptPrintBill.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            PrintUtils mPrintUtils = new PrintUtils(SucessReceiptPrintBill.this, mPrinter, bitMap,SucessReceiptPrintBill.this);
            mPrintUtils.print_Header(responseListForPrinter_Header);
            mPrintUtils.print_PrintbillPay(responseListForPrinter_Body);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(SucessReceiptPrintBill.this);
        try {
    ...
    private void generateAndSetData() {
        ...
        responseListForPrinter_Header.add(1, recieptData[12].split("\\ ")[0]);
        responseListForPrinter_Header.add(2, recieptData[12].split("\\ ")[1]);
        responseListForPrinter_Header.add(3, recieptData[15]);
        responseListForPrinter_Header.add(4, getString(R.string.billPay));

        responseListForPrinter_Body.add(0, recieptData[6]); // Marchant Name
        responseListForPrinter_Body.add(1, billNumber[0]);      // Bill Number
        responseListForPrinter_Body.add(2, recieptData[7]);      // Label Name
        responseListForPrinter_Body.add(3, recieptData[8]);      // Cutomer ID
        responseListForPrinter_Body.add(4, customerNumberArray[1]);     // Customer number :
        responseListForPrinter_Body.add(5, recieptData[6]);   // Customer Name

        if(recieptData[10].equalsIgnoreCase("null")) {
            recieptData[10] = "0.0";

            responseListForPrinter_Body.add(6, arrayData[1]);  // Amount
            responseListForPrinter_Body.add(7, recieptData[10]);  // fees
            responseListForPrinter_Body.add(8, arrayData[1]);  // Total Amount
        ...
        responseListForPrinter_Body.add(10, recieptData[3]); // Agent Branch Name
    ...
    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(SucessReceiptPrintBill.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(SucessReceiptPrintBill.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(SucessReceiptPrintBill.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(SucessReceiptPrintBill.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
