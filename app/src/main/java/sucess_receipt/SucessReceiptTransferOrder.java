...
public class SucessReceiptTransferOrder extends AppCompatActivity implements TaskCompleteToPrint {
    TextView dateRecepit, timeReceipt, energeticValue, energyConst, customerNameReceipt, enoTransactionNumberReceipt, totalamountRecepit, meterIDReceipt, senderNameReceipt, commentsTitleReview, destinationNameReceipt, destinationNumberReceipt, transactionCodeRecepit, transactionIdReceipt, benificeAccountNumberTextview, enelNumberReceipt, agentBranceNameReceipt, cityReceipt, labelNameReceipt, amountRecepit, feesRecepit, agentNameRecepit, agentCountryRecepit;
    Button homeButton;
    String[] recieptData;
    ComponentMd5SharedPre mComponentInfo;
    String agentCode, commentString,benificeAccountNumberString, agentBranchString,countrySelectionString, amountString,agentName;
    ...
    SucessReceiptTransferOrder.ConnectToPrinter connect_to_Printer;
    String printdata;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.success_receipt_transfer_order);
        Toolbar mToolbar;
        ...
        benificeAccountNumberTextview = (TextView) findViewById(R.id.benificeAccountNumberTextview);
        ...
        benificeAccountNumberString = getIntent().getExtras().getString("benificeAccountNumberString", "");
        amountString = getIntent().getExtras().getString("amountString", "");
        countrySelectionString = getIntent().getExtras().getString("countrySelectionString", "");
        commentString = getIntent().getExtras().getString("commentString", "");
      ...
      //        enoTransactionNumberReceipt.setText(recieptData[8]);
       ...
       /* float amountAdd = Float.parseFloat(commentString);
        ...
        benificeAccountNumberTextview.setText(benificeAccountNumberString);
        // senderNameReceipt.setText(senderNameString);
     //   agentBranceNameReceipt.setText(" Agent Branch from server");
        //  customerNameReceipt.setText(customerName);
        agentNameRecepit.setText(agentName);
        agentCountryRecepit.setText(countrySelectionString);
        ...
        homeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SucessReceiptTransferOrder.this.finish();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
                    if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                        connect_to_Printer = new SucessReceiptTransferOrder.ConnectToPrinter();
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(SucessReceiptTransferOrder.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, SucessReceiptTransferOrder.this);
                        ...
                        a++;
                        status = 3;
                        SucessReceiptTransferOrder.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(SucessReceiptTransferOrder.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    SucessReceiptTransferOrder.PrintToPrinter printToPrinter = new SucessReceiptTransferOrder.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(SucessReceiptTransferOrder.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = SucessReceiptTransferOrder.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            PrintUtils mPrintUtils = new PrintUtils(SucessReceiptTransferOrder.this, mPrinter, bitMap, SucessReceiptTransferOrder.this);
            mPrintUtils.print_Header(responseListForPrinter_Header);
            mPrintUtils.print_orderTransfer(responseListForPrinter_Body);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(SucessReceiptTransferOrder.this);
        try {
    ...
    private void generateAndSetData() {
        ...
        responseListForPrinter_Header.add(4, getString(R.string.orderTransfer_capital));

        // BODY


        responseListForPrinter_Body.add(0, benificeAccountNumberString);  // benifice Account Number
        responseListForPrinter_Body.add(1, amountString);  // Amount
        responseListForPrinter_Body.add(2, agentName); // Agent name
      //  responseListForPrinter_Body.add(3,  "Agent Branch From server"); // Attach Branch Name
        responseListForPrinter_Body.add(3, countrySelectionString); // Attach Branch Name
        responseListForPrinter_Body.add(4, commentString); // Comments

    }

    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(SucessReceiptTransferOrder.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(SucessReceiptTransferOrder.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(SucessReceiptTransferOrder.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(SucessReceiptTransferOrder.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
