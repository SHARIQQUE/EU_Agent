...
public class SucessReceiptReprintReceiveMoneyToMobile extends AppCompatActivity implements TaskCompleteToPrint {
    ...
    Button homeButton;
    String[] recieptData;
    Double feesNull, feesAdd, totalAmount, amountAdd, taxAdd;
    ...
    int a = 0;
    String totalAmountString,totalFeesString;
    private PrinterManager mPrinterManager;
    private Handler mPrinterConnectionHandler;
    private String printMessage;
    private boolean doPrint = false;
    private BluetoothAdapter btAdapter;
    SucessReceiptReprintReceiveMoneyToMobile.ConnectToPrinter connect_to_Printer;
    String printdata;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sucess_receipt_reprint_receivemoney_tomobile);
        Toolbar mToolbar;
...
*/
            ...
            String dateTimeSaperate = recieptData[11];
            String[] dateTime = dateTimeSaperate.split("\\ ");
            String date = dateTime[0];
            String time = dateTime[1];
            dateRecepit.setText(date);
            timeReceipt.setText(time);
            destinationCountryRecepit.setText(recieptData[8]);
            commentsTitleReview.setText(recieptData[12]);

            if (recieptData[7].equalsIgnoreCase("null")) {
                feesNull = 0.0;
                feesRecepit.setText("0.0");
                amountRecepit.setText(recieptData[9]);
                totalamountRecepit.setText(recieptData[9]);
            } else {


          /*  amountAdd = Double.parseDouble(recieptData[9]);
            feesAdd = Double.parseDouble(recieptData[7]);
            totalAmount = (amountAdd - feesAdd);*/


                amountAdd = Double.parseDouble(recieptData[9]);
                feesAdd = Double.parseDouble(recieptData[7]);
                taxAdd = Double.parseDouble(recieptData[15]);

                totalAmount = (amountAdd + feesAdd + taxAdd);
                ...
                Double  feesDoubleTemp = (feesAdd + taxAdd);
                totalFeesString = Double.toString(feesDoubleTemp);

                feesRecepit.setText(totalFeesString);
                amountRecepit.setText(totalAmountString);
                totalamountRecepit.setText(recieptData[9]);    // from server
            }

            currencyTypeReceipt.setText(recieptData[14]);  //  coming from serber 2 dec 2017
            destinationNumberReceipt.setText(recieptData[5]);
            destinationNameReceipt.setText(recieptData[4]);
            // transactionCodeRecepit.setText(recieptData[14]);
            attachBranceNameReceipt.setText(recieptData[6]);
            agentNameRecepit.setText(recieptData[10]);
            agentCountryRecepit.setText(recieptData[8]);

            oldTxnIdLL.setVisibility(View.VISIBLE);
            oldTxnidTextViewReceipt.setText(recieptData[13]);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
      ...
      */
        ...
        homeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SucessReceiptReprintReceiveMoneyToMobile.this.finish();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
                    if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                        connect_to_Printer = new SucessReceiptReprintReceiveMoneyToMobile.ConnectToPrinter();
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            PrintUtils mPrintUtils = new PrintUtils(SucessReceiptReprintReceiveMoneyToMobile.this, mPrinter, bitMap, SucessReceiptReprintReceiveMoneyToMobile.this);
            mPrintUtils.print_Header(responseListForPrinter_Header);
            mPrintUtils.print_ReprintReceiveMoneyToMobile(responseListForPrinter_Body);
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(SucessReceiptReprintReceiveMoneyToMobile.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, SucessReceiptReprintReceiveMoneyToMobile.this);
                        ...
                        a++;
                        status = 3;
                        SucessReceiptReprintReceiveMoneyToMobile.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(SucessReceiptReprintReceiveMoneyToMobile.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    SucessReceiptReprintReceiveMoneyToMobile.PrintToPrinter printToPrinter = new SucessReceiptReprintReceiveMoneyToMobile.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(SucessReceiptReprintReceiveMoneyToMobile.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = SucessReceiptReprintReceiveMoneyToMobile.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(SucessReceiptReprintReceiveMoneyToMobile.this);
        try {
    ...
    private void generateAndSetData() {
        ...
        responseListForPrinter_Header.add(1, recieptData[11].split("\\ ")[0]);
        responseListForPrinter_Header.add(2, recieptData[11].split("\\ ")[1]);
        ...
        responseListForPrinter_Body.add(0, recieptData[13]);   // Old
        responseListForPrinter_Body.add(1, recieptData[8]);   // Destination Country
        ...
        responseListForPrinter_Body.add(4, recieptData[14]); // currency Type

        responseListForPrinter_Body.add(5, totalAmountString);       // Amount
        responseListForPrinter_Body.add(6, totalFeesString);        // fees
        responseListForPrinter_Body.add(7, recieptData[9]);       // Total Amount

        responseListForPrinter_Body.add(8, recieptData[10]);           // Agent Name
        responseListForPrinter_Body.add(9, recieptData[6]);     // Agent Branch NAme
        responseListForPrinter_Body.add(10, recieptData[8]); // Agent Country
        responseListForPrinter_Body.add(11, recieptData[12]); // Agent Country
    ...
    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(SucessReceiptReprintReceiveMoneyToMobile.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(SucessReceiptReprintReceiveMoneyToMobile.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(SucessReceiptReprintReceiveMoneyToMobile.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(SucessReceiptReprintReceiveMoneyToMobile.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
