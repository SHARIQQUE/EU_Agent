...
public class SucessReceiptCashOutDiffrentBranch extends AppCompatActivity implements TaskCompleteToPrint {
    TextView dateRecepit,commentRecepit, timeReceipt, transactionIdReceipt, subscriberNumberReceipt, subscribeNameReceipt, attachBranceNameReceipt, cityReceipt, countryRecepit, amountRecepit, feesRecepit, agentNameRecepit, agentCountryRecepit;
    Button homeButton;
    Double feesNull;
    String comment_cashout="";
    String[] recieptData;
    ComponentMd5SharedPre mComponentInfo;
    String stateNameString, agentCode,agentCountry,agentName;
    Bundle bundle = new Bundle();
    String total_fees_tax,subscriberNoDestinationNo;
    private ProgressDialog mDialog;
    private ProtocolAdapter mProtocolAdapter;
    String amountCashOutMenu,readerAddress, printerAddress;
    ...
    SucessReceiptCashOutDiffrentBranch.ConnectToPrinter connect_to_Printer;
    String printdata;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sucess_receipt_cashout_diffrent_branch);
        Toolbar mToolbar;


        commentRecepit = (TextView) findViewById(R.id.commentRecepit);
        ...
        mToolbar = (Toolbar) findViewById(R.id.tool_bar_transactionReceiptDiffrentBranch);
        ...
        agentCountry = getIntent().getExtras().getString("agentCountry", "");
        stateNameString = mComponentInfo.getmSharedPreferences().getString("stateNameString", "");
        amountCashOutMenu = mComponentInfo.getmSharedPreferences().getString("amountCashOutMenu", "");


        comment_cashout = mComponentInfo.getmSharedPreferences().getString("comment_cashout", "");
        commentRecepit.setText(comment_cashout);
        ...
        try {
            ...
            if (recieptData[6].equalsIgnoreCase("") || recieptData[6].equalsIgnoreCase("null")) {
                feesNull = 0.0;
                feesRecepit.setText("0.0");
                amountRecepit.setText(recieptData[7]);
            } else {

                Double fees = Double.parseDouble(recieptData[6]);  // fees
                Double tax = Double.parseDouble(recieptData[11]);  // tax

                Double total_fees_temp = (fees + tax);
                total_fees_tax = Double.toString(total_fees_temp);

                feesRecepit.setText(total_fees_tax);
                amountRecepit.setText(recieptData[7]);
            }
        }
        catch (Exception e)
        ...
        agentCountryRecepit.setText(agentCountry);


        Button printButton = (Button) findViewById(R.id.printButton);
        int amountGreter=Integer.parseInt(amountCashOutMenu);

        if(amountGreter > 500000)
        {
        ...
        homeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SucessReceiptCashOutDiffrentBranch.this.finish();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
                    if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                        connect_to_Printer = new SucessReceiptCashOutDiffrentBranch.ConnectToPrinter();
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(SucessReceiptCashOutDiffrentBranch.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, SucessReceiptCashOutDiffrentBranch.this);
                        ...
                        a++;
                        status = 3;
                        SucessReceiptCashOutDiffrentBranch.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(SucessReceiptCashOutDiffrentBranch.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    SucessReceiptCashOutDiffrentBranch.PrintToPrinter printToPrinter = new SucessReceiptCashOutDiffrentBranch.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(SucessReceiptCashOutDiffrentBranch.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = SucessReceiptCashOutDiffrentBranch.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            PrintUtils mPrintUtils = new PrintUtils(SucessReceiptCashOutDiffrentBranch.this, mPrinter, bitMap,SucessReceiptCashOutDiffrentBranch.this);
            mPrintUtils.print_Header(responseListForPrinter_Header);
            mPrintUtils.print_cashDiffrentBranch(responseListForPrinter_Body);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(SucessReceiptCashOutDiffrentBranch.this);
        try {
    ...
    private void generateAndSetData() {
        ...
        responseListForPrinter_Header.add(4, getString(R.string.cashOutDiffrentBranchReceipt));
        ...
        responseListForPrinter_Body.add(3, stateNameString);  // City number
        ...
        try {
            ...
            if (recieptData[6].equalsIgnoreCase("") || recieptData[6].equalsIgnoreCase("null")) {
                ...
                responseListForPrinter_Body.add(6, total_fees_tax);  // fees
            }
        }
        catch (Exception e)
        {
        ...
        responseListForPrinter_Body.add(7, comment_cashout);  // Agent NAme
        responseListForPrinter_Body.add(8, agentName);  // Agent NAme
        responseListForPrinter_Body.add(9, agentCountry);   // Agent Country
    }

    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(SucessReceiptCashOutDiffrentBranch.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(SucessReceiptCashOutDiffrentBranch.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(SucessReceiptCashOutDiffrentBranch.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(SucessReceiptCashOutDiffrentBranch.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
    @Override
