...
public class SucessReceiptCashToCashReceiveMoneySameCountryNew extends AppCompatActivity implements TaskCompleteToPrint {
    ...
    String amountSearchNoToWord,amountSearch,SenderNameAgentIdentity, destinationNameAgentIdentity;
    ComponentMd5SharedPre mComponentInfo;
    String agentCode, commentString, destinationNoString, agentName, spinnerCurrenceyString;

    Bundle bundle = new Bundle();
    String destinationNameStringNew,senderNameString,currencySearchString,senderNumberString,subscriberNoDestinationNo;
    ...
    SucessReceiptCashToCashReceiveMoneySameCountryNew.ConnectToPrinter connect_to_Printer;
    String printdata;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.success_receipt_cashtocash_receivemoney_samecountry_new);
        Toolbar mToolbar;
        Button printButton;
        ...
        destinationNameStringNew = mComponentInfo.getmSharedPreferences().getString("destinationNameStringNew", "");
        ...
        destinationNameReceipt.setText(destinationNameStringNew);
        attachBranceNameReceipt.setText(recieptData[6]);
        agentNameRecepit.setText(agentName);
        agentCountryRecepit.setText(recieptData[7]);
        destinationCountryRecepit.setText(recieptData[7]);
        commentsTitleReview.setText(commentString);
        referenceNumber_textView.setText(recieptData[8]);
       ...
       generateAndSetData();   // print
        ...
        homeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SucessReceiptCashToCashReceiveMoneySameCountryNew.this.finish();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if (askForPermission(Manifest.permission.BLUETOOTH, REQUEST_BLUETOOTH_PHONE)) {
                    if (askForPermission(Manifest.permission.BLUETOOTH_ADMIN, REQUEST_BLUETOOTH_PHONE_ADMIN)) {
                        connect_to_Printer = new SucessReceiptCashToCashReceiveMoneySameCountryNew.ConnectToPrinter();
    ...
    private class PrintToPrinter extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            PrintUtils mPrintUtils = new PrintUtils(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, mPrinter, bitMap, SucessReceiptCashToCashReceiveMoneySameCountryNew.this);
            mPrintUtils.print_Header(responseListForPrinter_Header);
            mPrintUtils.print_cashToCashReceiveMoneySameCountry(responseListForPrinter_Body);
    ...
    private class ConnectToPrinter extends AsyncTask<String, Void, String> {
                    ...
                    try {
                        ...
                        if (printerAddress != null) {
                            ...
                            if (a < 2) {
                                ...
                                mPrinterManager = PrinterManager.getInstance(SucessReceiptCashToCashReceiveMoneySameCountryNew.this);
                                mPrinterManager.disconnect();
                                Thread.sleep(1500);
                                mPrinterManager.connect(printerAddress, SucessReceiptCashToCashReceiveMoneySameCountryNew.this);
                        ...
                        a++;
                        status = 3;
                        SucessReceiptCashToCashReceiveMoneySameCountryNew.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (status) {
                case 0:
                    hideProgressDialog();
                    Toast.makeText(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, "Bluetooth not supported", Toast.LENGTH_SHORT).show();
                    break;
                ...
                case 2:
                    ...
                    SucessReceiptCashToCashReceiveMoneySameCountryNew.PrintToPrinter printToPrinter = new SucessReceiptCashToCashReceiveMoneySameCountryNew.PrintToPrinter();
                ...
                case 3:
                    hideProgressDialog();
                    // commonDisplay.dismissProgressDialog();
                    startActivityForResult(new Intent(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, PrinterListShowActivity.class), 300);
                    break;
            }
        }
        ...
        protected void onPreExecute() {
            ...
            SharedPreferences sp = SucessReceiptCashToCashReceiveMoneySameCountryNew.this.getSharedPreferences(CommonUtilities.PREFFS_NAME, 0);
    ...
    private void showProgressDialog(String message) {
        ...
        mDialog = new ProgressDialog(SucessReceiptCashToCashReceiveMoneySameCountryNew.this);
        try {
    ...
    private void generateAndSetData() {
        ...
        responseListForPrinter_Header.add(4, getString(R.string.cashToCashReceiveMoneyNew));
        ...
        responseListForPrinter_Body.add(2, recieptData[7]);             // Destination Country
        responseListForPrinter_Body.add(3, destinationNameStringNew);      // destination name
        ...
        responseListForPrinter_Body.add(9, recieptData[6]);   // Agnet branch Name
        responseListForPrinter_Body.add(10, recieptData[7]);  // Agent Country
        responseListForPrinter_Body.add(11, commentString);  // comment
    }

    private boolean askForPermission(String permission, Integer requestCode) {
        boolean ret = false;
        if (ContextCompat.checkSelfPermission(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, permission) != PackageManager.PERMISSION_GRANTED) {

            // Should we show an explanation?
            if (ActivityCompat.shouldShowRequestPermissionRationale(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, permission)) {

                //This is called if user has denied the permission before
                //In this case I am just asking the permission again
                ActivityCompat.requestPermissions(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, new String[]{permission}, requestCode);
...
//            }
            ...
            e.printStackTrace();
            Toast.makeText(SucessReceiptCashToCashReceiveMoneySameCountryNew.this, getString(R.string.pleaseTryAgainLater), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
